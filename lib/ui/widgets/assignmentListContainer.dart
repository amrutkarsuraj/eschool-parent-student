    ...
    if (selectedAssignmentFilter == AssignmentFilters.dueDateLatest) {
      ...
      sortedAssignments.sort((first, second) {
        DateTime? firstDueDate =
            Utils.parseAssignmentDueDate(first.dueDate, first.dateFormat);
        DateTime? secondDueDate =
            Utils.parseAssignmentDueDate(second.dueDate, second.dateFormat);

        if (firstDueDate == null && secondDueDate == null) return 0;
        if (firstDueDate == null) return 1;
        if (secondDueDate == null) return -1;

        return secondDueDate.compareTo(firstDueDate);
      });
      ...
      sortedAssignments.sort((first, second) {
        DateTime? firstAssignedDate = Utils.parseApiDate(first.createdAt);
        DateTime? secondAssignedDate = Utils.parseApiDate(second.createdAt);

        if (firstAssignedDate == null && secondAssignedDate == null) return 0;
        if (firstAssignedDate == null) return 1;
        if (secondAssignedDate == null) return -1;

        return secondAssignedDate.compareTo(firstAssignedDate);
      });
    } else if (selectedAssignmentFilter ==
        AssignmentFilters.assignedDateOldest) {
      // Sort by assigned date (oldest first)
      sortedAssignments.sort((first, second) {
        DateTime? firstAssignedDate = Utils.parseApiDate(first.createdAt);
        DateTime? secondAssignedDate = Utils.parseApiDate(second.createdAt);
        ...
        return firstAssignedDate.compareTo(secondAssignedDate);
      });
    }
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const Spacer(),
                              Text(
                                "${Utils.getTranslatedLabel(dueKey)} : ${assignment.dueDate}",
                                style: TextStyle(
                                  color:
