...
class _ChildFeeDetailsScreenState extends State<ChildFeeDetailsScreen> {
  ...
  //   }
  // }
  // Fixed Razorpay payment method in your ChildFeeDetailsScreen

 // Fixed Razorpay payment method in your ChildFeeDetailsScreen

void payWithRazorpay({required String razorpayApiKey}) async {
  try {
    final orderId = context.read<PrePaymentTasksCubit>().getRazorpayOrderId();
    final amountToPay = context.read<PrePaymentTasksCubit>().getRazorpayAmountToPay();
    
    var options = {
      'key': razorpayApiKey,
      'amount': amountToPay.toInt(), // Don't multiply by 100 - already in paise
      'order_id': orderId,
      'name': 'Demo School',
      'prefill': {
        'contact': context.read<AuthCubit>().getParentDetails().mobile ?? "",
...
// Enhanced error handling for Razorpay callbacks
void handleRazorpayPaymentSuccess(PaymentSuccessResponse response) {
  if (kDebugMode) {
  ...
  // You might want to store these in PrePaymentTasksCubit or pass to confirm screen
  navigateToConfirmPaymentScreen();
}
...
void handleRazorpayPaymentError(PaymentFailureResponse response) {
  ...
  String errorMessage = _getRazorpayErrorMessage(response.code, response.message);
...
String _getRazorpayErrorMessage(int? code, String? message) {
  switch (code) {
    case 0:
      return "Payment was cancelled";
    case 1:
      return "Payment failed due to network error. Please check your internet connection.";
    case 2:
      return "Payment failed due to invalid payment details. Please try again.";
    case 3:
      return "Payment gateway error occurred. Please try again.";
    case 4:
  ...
  //   );
  // }
Widget _buildPayNowButton({double? advanceAmount, List<int>? installmentIds}) {
  if (kDebugMode) {
    print("=== BUILDING PAY BUTTON ===");
    print("Advance Amount: $advanceAmount");
    print("Installment IDs: $installmentIds");
  }
  ...
  return BlocConsumer<PrePaymentTasksCubit, PrePaymentTasksState>(
    ...
    builder: (context, paymentTaskState) {
      ...
      return PopScope(
        canPop: (paymentTaskState is! PrePaymentTasksInProgress),
        child: CustomRoundedButton(
          height: 35,
          radius: 5.0,
          widthPercentage: 0.9,
