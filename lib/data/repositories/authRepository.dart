...
class AuthRepository {
  ...
  final rawData = box.get(studentProfileDataKey) ??
                  box.get(studentDetailsKey) ??
                  {};

  print("Raw student profile data: $rawData");

  return Student.fromJson(Map.from(rawData));
}
...
Future<Map<String, dynamic>> signInStudent({
  ...
  try {
    ...
    return {
      "jwtToken": result['token'],
      "schoolCode": schoolCode, // input से ही use करें
      "student": Student.fromJson(Map.from(result['data']))
    };
  } catch (e) {
    if (kDebugMode) {
